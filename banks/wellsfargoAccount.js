...
      .trim();
    ...
    // The single requested change: Remove "authorized on [date]" and everything before it
    // Regex matches the start of the string (^), any characters (.*) followed by 
    // "authorized on", a date (1-2 digits/1-2 digits), and any trailing spaces (\s*).
    // The 'i' flag ensures case-insensitivity.
    fullDescription = fullDescription.replace(/^.*authorized\s+on\s+\d{1,2}\/\d{1,2}\s*/i, '').trim();
...
        })
        .then(pagesText => {
          // Combine all page text and extract transactions
          const fullText = pagesText.join(' ');
          const transactions = extractTransactionsFromPDF(fullText);
...
  // This regex looks for date patterns followed by transaction descriptions and amounts
  const transactionRegex = /(\d{1,2}\/\d{1,2})\s+([^$]+?)\s+([\d,]+\.\d{2})/g;
  
  const transactions = [];
  let match;
  
...
    
    // Clean up the description - remove extra spaces and noise
...
        description.includes('Deposits/Additions') ||
        description.includes('Withdrawals/Subtractions') ||
        description.includes('Transaction history') ||
