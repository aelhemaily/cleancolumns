...
function parseTransactionsFromText(text) { 
 ...
 if (transaction) { 
 // Filter out transactions with $0.00 amount
...
window.bankUtils.processPDFFile = async function(file) { 
 ...
 let balanceToFormat = tx.balance; 
 // Check if balance is negative and not already in parentheses 
 if (balanceToFormat.startsWith('-') && !balanceToFormat.startsWith('(')) { 
 balanceToFormat = `(${balanceToFormat.substring(1)})`; 
 } 
 return `${tx.date} ${tx.description} ${tx.amount} ${balanceToFormat}`; 
 }).join('\n'); 
...
function processTangerineData() { 
 ...
 // Filter out transactions with $0.00 amount
 if (parsedTransaction.amount !== '0.00') {
 ...
 // FIX: Format the balance string with parentheses if it's negative 
 let formattedBalance = item.balance; 
 if (currentBalanceValue < 0) { 
 // Remove the negative sign and enclose in parentheses 
 formattedBalance = `(${Math.abs(currentBalanceValue).toFixed(2)})`; 
 } else { 
 formattedBalance = currentBalanceValue.toFixed(2); 
 } 

 rows.push([date, description, debit, credit, formattedBalance]); // Use formattedBalance for the display column 
